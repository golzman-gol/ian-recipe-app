...
import { ArrowLeft, Clock, ChefHat, Trash2, Pencil, Link, Share, Download, Check, X, ExternalLink } from 'lucide-react';
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 pb-32">
      ...
      <div className="bg-white rounded-3xl p-6 mb-8 border border-zinc-200 shadow-sm">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2 bg-zinc-100 p-1 rounded-full">
            {[0.5, 1, 2].map(m => (
              <button key={m} onClick={() => setMultiplier(m)} className={`px-4 py-1.5 rounded-full text-sm font-bold ${multiplier === m ? 'bg-zinc-900 text-white' : 'text-zinc-500'}`}>{m}x</button>
            ))}
          </div>
        </div>
        <ul className="space-y-3">
          {scaledIngredients.map((ing, idx) => (
            <li key={idx} className="flex gap-4 border-b border-zinc-50 pb-3 last:border-0">
              <span className="font-mono font-bold w-16 text-right">{ing.amount.toFixed(ing.amount % 1 === 0 ? 0 : 2)}</span>
              <span className="text-zinc-500 w-16">{ing.unit}</span>
              ...
              return (
                <div key={idx} className="bg-white border rounded-2xl overflow-hidden shadow-sm flex flex-col">
                  ...
                  <div className="p-3 bg-zinc-50 flex-1">
                    <a href={yt?.direct || video.url} target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold text-zinc-900 bg-white border border-zinc-200 px-2 py-1.5 rounded block text-center mb-2 hover:bg-zinc-50 transition-colors">OPEN IN YOUTUBE</a>
      ...
      <div className="border-t pt-8">
        ...
        <div className="flex gap-3 mb-8">
          <input type="text" value={newNote} onChange={(e) => setNewNote(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddNote()} placeholder="Add a note..." className="flex-1 border rounded-2xl px-5 py-3 bg-zinc-50 outline-none focus:ring-2 focus:ring-zinc-900" />
            ...
            <div key={note.id} className="bg-white border rounded-2xl p-5 shadow-sm relative">
              {editingNoteId === note.id ? (
                <div className="space-y-3">
                  <textarea value={editingNoteText} onChange={(e) => setEditingNoteText(e.target.value)} className="w-full border rounded-xl p-3 bg-zinc-50" rows={3} />
                  <div className="flex gap-2">

      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-zinc-500 mb-2">מנות בסיס (Base Servings)</label>
              <input
                type="number"
              ...
              <div className="absolute z-10 w-full mt-2 bg-white border border-zinc-200 rounded-2xl shadow-lg max-h-60 overflow-y-auto">
                {filteredTags.map(tag => (
                  <button
                    key={tag}
                    onClick={() => addTag(tag)}
                    className="w-full text-right px-4 py-3 hover:bg-zinc-50 focus:bg-zinc-50 focus:outline-none"
                  >
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 bg-zinc-50 border border-zinc-200 rounded-xl p-1">
                <input
                  ...
                  placeholder="הכפל ב-"
                />
                <button onClick={applyScale} className="px-4 py-2 bg-zinc-900 text-white rounded-lg text-sm font-medium hover:bg-zinc-800 transition-colors">שנה הכל</button>
              </div>
              <button onClick={addIngredient} className="flex items-center gap-1 text-sm font-medium text-zinc-600 hover:text-zinc-900 bg-zinc-100 px-4 py-2.5 rounded-xl transition-colors">
          ...
          <div className="space-y-3">
            {recipe.ingredients.map((ing, idx) => (
              <div key={idx} className="flex gap-2 items-center bg-zinc-50/50 p-2 rounded-2xl border border-zinc-100 group">
                ...
                />
                <button onClick={() => removeIngredient(idx)} className="p-3 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors">
                  <Trash2 className="w-5 h-5" />
                </button>
              </div>
            ))}
        ...
        <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={addStep} className="flex items-center gap-1 text-sm font-medium text-zinc-600 hover:text-zinc-900 bg-zinc-100 px-3 py-1.5 rounded-full">
              <Plus className="w-4 h-4" /> הוסף
            </button>
          </div>
              ...
              <div key={idx} className="flex flex-col gap-2">
                {idx > 0 && (
                  <div className="flex justify-center -my-2 relative z-10">
                    <button onClick={() => insertStep(idx)} className="p-1 bg-white border border-zinc-200 rounded-full text-zinc-400 hover:text-zinc-900 hover:border-zinc-400 shadow-sm transition-colors" title="הוסף שלב כאן">
                ...
                <div className="flex gap-3 items-start">
                  <div className="flex flex-col items-center gap-1 mt-1">
                    ...
                    <div className="flex flex-col gap-1 mt-1">
                      <button onClick={() => moveStepUp(idx)} disabled={idx === 0} className="p-1 text-zinc-400 hover:text-zinc-900 disabled:opacity-30 transition-colors">
                        <ArrowUp className="w-4 h-4" />
                      </button>
                      <button onClick={() => moveStepDown(idx)} disabled={idx === recipe.steps.length - 1} className="p-1 text-zinc-400 hover:text-zinc-900 disabled:opacity-30 transition-colors">
                        <ArrowDown className="w-4 h-4" />
                      </button>
                    </div>
                  </div>
                  ...
                  />
                  <button onClick={() => removeStep(idx)} className="p-3 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors mt-1">
                    <Trash2 className="w-5 h-5" />
                  </button>
                </div>
              </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => processImagesInputRef.current?.click()} className="flex items-center gap-1 text-sm font-medium text-zinc-600 hover:text-zinc-900 bg-zinc-100 px-3 py-1.5 rounded-full">
                ...
                return (
                  <div key={idx} className="flex flex-col gap-2">
                    <div className="relative aspect-video rounded-2xl overflow-hidden border border-zinc-200">
                      ...
                      <button onClick={() => removeProcessImage(idx)} className="absolute top-2 left-2 p-1.5 bg-black/50 text-white rounded-full hover:bg-black/70 backdrop-blur-sm">
        ...
        <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm space-y-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={addReferenceVideo} className="flex items-center gap-1 text-sm font-medium text-zinc-600 hover:text-zinc-900 bg-zinc-100 px-3 py-1.5 rounded-full">
              <Plus className="w-4 h-4" /> הוסף
            </button>
          </div>
          <div className="space-y-4">
            {(recipe.reference_videos || []).map((video, idx) => (
              <div key={idx} className="flex gap-4 items-start bg-zinc-50 p-4 rounded-2xl border border-zinc-200">
                ...
                <button onClick={() => removeReferenceVideo(idx)} className="p-3 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors">
                  <Trash2 className="w-5 h-5" />
                </button>
              </div>
            ))}
        ...
        {/* דגשים קולינריים */}
        <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm">
          <label className="block text-lg font-semibold text-zinc-900 mb-2">דגשים קולינריים</label>
          <p className="text-sm text-zinc-500 mb-4">טיפים נוספים או הסברים מדעיים...</p>
          <textarea
        ...
        <div className="flex gap-4 pt-4">
          <button onClick={onCancel} className="flex-1 py-4 px-4 border border-zinc-200 rounded-2xl shadow-sm text-base font-medium text-zinc-700 bg-white hover:bg-zinc-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zinc-900 transition-all active:scale-[0.98]">ביטול</button>
          <button onClick={handleSave} className="flex-1 flex justify-center items-center gap-2 py-4 px-4 border border-transparent rounded-2xl shadow-sm text-base font-medium text-white bg-zinc-900 hover:bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zinc-900 transition-all active:scale-[0.98]">
